...
def dct_2d(x, norm=None):
    ...
    """
    X1 = dct_torch(x, norm=norm)
    X2 = dct_torch(X1.transpose(-1, -2), norm=norm)
...
def idct_2d(X, norm=None):
    ...
    :return: the DCT-II of the signal over the last 2 dimensions
    """
    x1 = idct_torch(X, norm=norm)
    x2 = idct_torch(x1.transpose(-1, -2), norm=norm)
    return x2.transpose(-1, -2)
...
def dct_3d(x, norm=None):
    ...
    X1 = dct_torch(x, norm=norm)
    X2 = dct_torch(X1.transpose(-1, -2), norm=norm)
    X3 = dct_torch(X2.transpose(-1, -3), norm=norm)
...
def idct_3d(X, norm=None):
    ...
    x1 = idct_torch(X, norm=norm)
    x2 = idct_torch(x1.transpose(-1, -2), norm=norm)
    x3 = idct_torch(x2.transpose(-1, -3), norm=norm)
    return x3.transpose(-1, -3).transpose(-1, -2)
...
class LinearDCT(nn.Linear):
    ...
    def reset_parameters(self):
        ...
        if self.type == 'dct1':
            ...
            self.weight.data = dct_torch(I, norm=self.norm).data.t()
        elif self.type == 'idct':
            self.weight.data = idct_torch(I, norm=self.norm).data.t()
...
"""if __name__ == '__main__':
    ...
    error = torch.abs(dct_torch(x) - linear_dct(x))
